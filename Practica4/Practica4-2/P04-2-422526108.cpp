...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::mat4(1.0f); // Inicializa la matriz de modelo para la escena

		// Prisma rectangular
		modelaux = model;
		model = glm::translate(model, glm::vec3(0.0f, 4.5f, -6.0f)); // Mover a la posición de la articulación 1 (ajustar según sea necesario)
		model = glm::scale(model, glm::vec3(4.0f, 2.0f, 2.0f)); // Escalar el prisma para que tenga el tamaño de la cabina
		color = glm::vec3(0.0f, 1.0f, 0.0f); // Color verde para la cabina
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model)); // Enviar la matriz de transformación
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); // Cambiar el color al verde
		meshList[0]->RenderMesh(); // Dibuja el cubo que representa la cabina
		model = modelaux;

		modelaux = model; // Guardamos la matriz antes de transformar el prisma
		model = glm::translate(model, glm::vec3(0.0f, 3.5f, -6.0f)); // Traslación del prisma
		model = glm::scale(model, glm::vec3(5.0f, 1.0f, 2.0f));
		color = glm::vec3(0.0f, 0.0f, 1.0f); // Color verde para el prisma
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos la matriz del prisma
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection)); // Enviamos la proyección
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix())); // Enviamos la vista
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); // Color para el prisma
		meshList[4]->RenderMesh(); // Dibuja el cubo que representa la cabina
		model = modelaux;

		//LLANTAS

		modelaux = model;
		model = glm::translate(model, glm::vec3(2.0f, 3.0f, -7.2f));
		...
		modelaux = model;
		model = glm::translate(model, glm::vec3(2.0f, 3.0f, -4.8f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
